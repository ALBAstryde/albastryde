...
// create media data for tables and median graphs
	...
	function make_graphs() {
		...
		if (editor_mode) {
			...
			function() {
				var state = $(this).attr('checked'), color_value = parseInt($(this).attr('name'), 10),i;
				for (i=0;i<converted_graphs.length;i++) {
...
//	}
			...
			function(data) {
				...
				if (data.status == 'success') {
					if ('remove' in data) {
						...
						comments[unique_pk][data.pk] = [comment, user_name, true, data.is_public];
					}
					redraw_graph();
	...
	function ranges_to_axis_dic(ranges) {
		...
		for (i=0;i<ranges.yaxis.length;i++) {
			axis_dic.yaxis.push({
